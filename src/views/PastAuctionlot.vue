    ...
    <div class="container mx-auto xl:px-4 lg:px-12 md:px-16 sm:px-8 px-2 py-10">
      <div class="flex">
        ...
        <div class="w-3/4">
          ...
          <div v-else-if="auctionItems.length === 0" class="text-center py-8">
          ...
          <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-6">
            <ProductCard v-for="lot in auctionItems" :key="lot.id" :product="formatProduct(lot)" />
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  mounted() {
    ...
    // Retrieve auction data from the Pinia store or session storage
    ...
    // Check if auction data exists; fetch details if available
    if (this.auctionId) {
      this.fetchAuctionDetails();
    } else {
      console.error('No auction data available. Redirecting to the fallback route.');
      this.$router.push({ name: 'PreviousAuctions' }); // Update with your fallback route
    }
  },
  methods: {
    async fetchAuctionDetails() {
      ...
      // Clear previous auction items to ensure fresh data is fetched
      this.auctionItems = [];
  ...
  watch: {
    auctionId(newVal) {
      if (newVal) {
        // Persist the auction data to session storage to handle reloads
